...
function radiale(angle, ecart, polygone) {
    ...
    var minx = polygone[0][0];
    var miny = polygone[0][1];
    ...
    var cotecarre = 2 * ecart + Math.sqrt(Math.pow(maxx - minx, 2) + Math.pow(maxy - miny, 2));
    var matrot = [[Math.cos(angle), -Math.sin(angle)], [Math.sin(angle), Math.cos(angle)]];
    var n = Math.floor(cotecarre / ecart) + 1;
    ...
    for (var i = 0; i < coordsegments.length; i++) {
        ...
        var distance = cotecarre / 500;
        var dehors = true;
        var test = false;
        var m = Math.floor(cotecarre / distance) + 1;
        for (var j = 0; j < m; j++) {
            ...
            if (test && dehors) {
                ...
                dehors = true;
                xfinsegment = point[0] - distance * Math.cos(angle);
                yfinsegment = point[1] - distance * Math.sin(angle);
